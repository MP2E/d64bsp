...
typedef struct raw_linedef_s
{
  ...
  uint8_g renderflags;
  uint8_g textureflags;
  uint8_g type;      // linedef type (0 for none, 97 for teleporter, etc)
  uint8_g usetype;
...
typedef struct raw_sidedef_s
{
  ...
  uint16_g upper_index;
  uint16_g lower_index;
  uint16_g mid_index;
...
typedef struct raw_sector_s
{
  ...
  uint16_g floor_index;
  uint16_g ceiling_index;

  uint16_g clr_ceiling;
  uint16_g clr_floor;
  uint16_g clr_thing;
  uint16_g clr_upper;
  uint16_g clr_lower;
  ...
  uint16_g tag;       // sector activated by a linedef with same tag
  uint16_g flag;
}
raw_sector_t;
